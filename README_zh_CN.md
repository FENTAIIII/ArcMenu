...
> 登录、音乐、公会、商店、任务、社交与副本引导，可以拥有同一种视觉语言。ArcMenu 为 Paper 服务端上的每套系统提供共同的界面基础，而普通玩家无需安装任何客户端模组。

ArcMenu 是一个开源、由服务端驱动的 Minecraft UI 框架。它把玩家私有的展示实体、固定屏幕平面、资源包图形、动画时间线、Tooltip 与公开应用 API 组合起来，让界面成为服务器体验的一部分。

它所面向的不只是“更多种菜单”。我们的目标是让一台服务器逐渐生长出自己的应用生态：账号、音乐、公会、商店、任务等接入插件继续掌管各自的数据与规则，ArcMenu 统一承担屏幕、输入、导航和生命周期。
...
<table>
  ...
  <tr>
    <td align="center"><strong>真正存在于游戏世界中的音乐</strong></td>
...
同一套基础还可以承载签到、邮件、拍卖、技能树、服务器设置、角色选择、任务日志和匹配大厅，也可以承载今天还没人设计出来的玩法。ArcMenu 负责让这些想法共享一块可靠的屏幕，而接入者无需反复实现摄像机、指针、实体所有权和返回栈。
...
- **严格配置：** 菜单坚持单一语义的 YAML 词汇；配置先校验再发布，重载失败时保留上一份有效运行状态。
...
完整契约见[第三方应用 API](docs/第三方应用API.md)，最小接入代码见 [Java 附属示例](examples/java-addon/src/main/java/example/ExampleArcMenuAddon.java)。
